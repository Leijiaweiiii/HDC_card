...
en_result_t FlashWriteData(uint8_t *Data, uint32_t *Addr, uint8_t len)
{
    ...
    for(i=0;i<len;i++)
    {
        ...
        enResult = Flash_WriteByte(*Addr,Data[i]);
        (*Addr)++;
    }
...
void FlashReadData(uint8_t *Data, uint32_t *Addr, uint8_t len)
{
    uint8_t i=0;
