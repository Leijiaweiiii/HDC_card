...
uint16_t Uart_CalScnt(uint8_t u8Idx,stc_uart_baud_t *pstcBaud)
{
    ...
    if((UartMode1 == pstcBaud->enRunMode)||(UartMode3 == pstcBaud->enRunMode))
    {
        ...
        u16Scnt = pstcBaud->u32Pclk/(pstcBaud->u32Baud*u8Over);//32位数据变为16位数据舍弃高16位
				//printf("pstcBaud->u32Baud = %d\n", (pstcBaud->u32Baud));
    }
    else
    {
        if(0 == u8Over)
...
en_result_t Uart_SendData(uint8_t u8Idx, uint8_t u8Data)
{
    ...
    if(Uart_GetStatus(u8Idx,UartFE) == TRUE)
    {
        //Uart_ClrStatus(u8Idx,UartFE);
    }        
    Uart_ClrStatus(u8Idx,UartTC);    
    return Ok;
}
 ...
 **\retval ErrorInvalidParameter发送失败
 ******************************************************************************/
en_result_t SkyTra_Send_Date(uint8_t u8Idx, uint8_t u8Data_buf[], uint16_t buf_len)
{
    static uint16_t i = 0;
    stc_uart_instance_data_t *pstcData = NULL;
    ASSERT(IS_VALID_CH(u8Idx));
    ...
    for(i=0; i<buf_len; i++)
    {
        pstcData->pstcInstance->SBUF_f.DATA = u8Data_buf[i];
        while(FALSE == Uart_GetStatus(u8Idx,UartTC))
        {
 ...
 **\retval ErrorInvalidParameter发送失败
 ******************************************************************************/
en_result_t Send_Command_Data(uint8_t u8Idx,const uint8_t* u8Data_buf)
{
    const uint8_t *data_buf = u8Data_buf;
    stc_uart_instance_data_t *pstcData = NULL;
    ASSERT(IS_VALID_CH(u8Idx));
    pstcData = UartGetInternDataPtr(u8Idx);
    if (NULL == pstcData)
    ...
    {
        Uart_ClrStatus(u8Idx,UartTC);
        pstcData->pstcInstance->SBUF_f.DATA = *data_buf;
        while(FALSE == Uart_GetStatus(u8Idx,UartTC))
        {
            if(Uart_GetStatus(u8Idx,UartFE) == TRUE)
            {
    ...
    send_gps_waittime = 200;
    send_gps_flag = 0;
    //Uart_EnableIrq(u8Idx,UartRxIrq);
    return Ok;
}
